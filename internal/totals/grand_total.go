package totals

import "context"

// GrandTotalCollector computes the final grand total.
// Sort order: 550 (always runs last).
//
// When price_includes_tax is active, product tax is already embedded in
// Subtotal, so only ShippingTaxAmount (from ShippingTaxCollector) is added.
type GrandTotalCollector struct{}

func (c *GrandTotalCollector) Code() string { return "grand_total" }

func (c *GrandTotalCollector) Collect(_ context.Context, _ *CollectorContext, total *Total) error {
	if total.TaxIncludedInPrice {
		// Product tax is already in Subtotal; only add shipping tax on top.
		total.GrandTotal = total.Subtotal - total.DiscountAmount + total.ShippingAmount + total.ShippingTaxAmount
	} else {
		total.GrandTotal = total.Subtotal - total.DiscountAmount + total.ShippingAmount + total.TaxAmount
	}
	return nil
}
